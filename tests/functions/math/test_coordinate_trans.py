...
from utils.functions.math.coordinate_trans import coordinate_transformation_in_angle, coordinate_transformation_in_position

class TestCoordinateTrans(object):
	def test_coordinate_transformation_in_angle(self):
		...
		assert round(math.sqrt((positions_1[0, 0])**2 + (positions_1[1, 0])**2), 5) == round(math.sqrt((translated_positions_1[0, 0])**2 + (translated_positions_1[1, 0])**2), 5)
		assert round(math.sqrt((positions_2[0, 0])**2 + (positions_2[1, 0])**2), 5) == round(math.sqrt((translated_positions_2[0, 0])**2 + (translated_positions_2[1, 0])**2), 5)
		assert round(math.sqrt((positions_2[0, 1])**2 + (positions_2[1, 1])**2), 5) == round(math.sqrt((translated_positions_2[0, 1])**2 + (translated_positions_2[1, 1])**2), 5)
	...
	def test_coordinate_transformation_in_position(self):
